...
\item if you want to merge germplasm name and selection name with the argument \texttt{merge_g_and_s} which is \texttt{TRUE} by default
\end{itemize}
...
Note that \texttt{NA} are put when there are no data for \texttt{bouquet} AND \texttt{vrac}.
