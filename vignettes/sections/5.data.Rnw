...
\item \texttt{query.type = "data-S"} to study selection differentia (subsection \ref{data-S-and-SR})
...
Each \texttt{query.type} is explained in the following sub-sections.
For each of them, the function returns a list with
\begin{itemize}
\item the data set with correlated variables
\item the data set with non correlated variables
\item the description of the methods used for each variable
\end{itemize}
...
colnames(data_classic$data$datasets.with.correlated.variables)
@
...
For a given trait, selection differentia corresponds to the difference between mean of the selected spikes and mean of the bulk (i.e. spikes that have not been selected).
Response to selection correponds to the difference between the mean of spikes coming from the selected spikes and the mean of spikes coming from the bulk (Figure \ref{SandR}).
...
colnames(data_S$data$datasets.with.correlated.variables)
@
...
The query takes into account when selection have been done in a seed lot, that this seed lot have been merged and then have been sown. It is the case when selection have been carried out in a replication that have been merged afterwards. Even if this case should not arise, it might happen.
...
colnames(data_SR$data$datasets.with.correlated.variables)
@
...
\texttt{x.axis} & 
\texttt{NULL} & 
factor displayed on the \texttt{x.axis}	 of a plot: \texttt{"germplasm"}, \texttt{"year"} or \texttt{"person"} referring to the attributes of a seed-lot. If \texttt{NULL}, all the combinaisons are done for \texttt{x.axis} and \texttt{in.col}. &
X &
X &
X &
  &
...
\texttt{in.col} & 
\texttt{NULL} & 
displayed in color of a plot: \texttt{"germplasm"}, \texttt{"year"} or \texttt{"person"} referring to the attributes of a seed-lot. If \texttt{NULL}, \texttt{in.col} is not displayed. Note that it is compulsory for data-biplot and data-radar as in these cases \texttt{x.axis} is not used. &
X &
X &
X &
X &
...
pm$`data-pie.on.map`$`spike_weight---spike_weight`$`map-[2013]`
@

\item Custom arguments

Arguments can be customized related to map plots (Table \ref{custom.map}).
...
\texttt{col\_to\_display} & \texttt{c("person", "germplasm", "year", "block", "X", "Y")} & columns to display in the table. It can be a vector with \texttt{"person"}, \texttt{"year"}, \texttt{"germplasm"}, \texttt{"block"}, \texttt{"X"} and \texttt{"Y"}. If \texttt{NULL}, none of these columns is displayed. The variables follow these columns. 
